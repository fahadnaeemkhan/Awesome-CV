...
%---------------------------------------------------------
  \cventry
    {Principal Software Architect} % Job title
    {Equinix} % Organization
    {Redwood city, CA} % Location
    {June. 2024 - Present} % Date(s)
    {
      \begin{cvitems} % Description(s) of tasks/responsibilities
        \item {Leading design and development of  different software products including:
          \begin{itemize}
            \item {Network Observability}
            \item {vNF vendor on boarding platform}
            \item {vNF performance test automation}
            \item {Vendor agnostic, network device validation \& certification platform}
            \item {Lab as a service (clabernetes based platform)}
          \end{itemize}
        }
        \item {Platform Engineering:
          \begin{itemize}
            \item {Redesigning and moving current services to be deployed on Kubernetes}
            \item {Maintaining and managing Kubernetes environment}
          \end{itemize}
        }
        \item {AI (just for fun :) ):
          \begin{itemize}
            \item {Experimenting with LLMs to help users with limited knowledge to get better error analysis and resolution}
            \item {Experimenting with RAG (Retrieval Augmented Generation) to setup a knowledge base for the platform to get better network CLIs for debugging}
            \item {Playing around designing MCP servers}
          \end{itemize}
        }
      \end{cvitems}
    }
  ...
  \cventry
    ...
    {March. 2021 - June. 2024} % Date(s)
    {
      \begin{cvitems} % Description(s) of tasks/responsibilities
        ...
        \item {Integrated ETO with TRIX (which provides network observability equinixmetal)}
